...
// BUG FIX: properly finds node before tail and returns new tail
struct Node* del_last_node(struct Node* tail) {
    ...
    while (prev->next != tail) { // find node just before tail
        prev = prev->next;
    }
    prev->next = tail->next;     // bypass old tail
    free(tail);
    return prev;                 // prev is now the new tail
}
